...
from transform_location import header2location,cal_block_size,generate_block_link_new
...
class HierarchicalTable:
    ...
    def generate_all_results(self):
        ...
        insight_vis_list=self.generate_blocks()
        # print("insight_vis_list[:10]")
        # print(insight_vis_list[:10])
        ...
        # if (self.data_source.source_name == "Console Sales"):      
        #     location_file = "link_result/all_link_1.txt"
        # if (self.data_source.source_name == "Console Sales(cumulative)"):      
        #     location_file = "link_result/all_link_2.txt"
        # with open(location_file, mode='w') as f:
        #     json.dump(result, f)
 ...
 '''
    ...
    def generate_blocks(self):
        ...
        args_list = []
        curr_focus_headers = []
        # print("self.curr_focus_all_state")
        # print(self.curr_focus_all_state)
        for i in range(len(self.curr_focus_all_state)):
            ...
            for idx in all_index:
                for col in all_columns:
                    ...
                    # print(curr_focus_headers)
                    # record the arguments of the block_processing function                     
                    args_list.append(args)
        self.args_list_=  args_list            
        print('processing blocks...')
        vis_list = []
        insight_vis_list = []
        
        for args in args_list:
            vis = self.process_block(args)
            vis_list.append(vis)
        ...
        for vis in vis_list:
            if vis:
                ...
                insight_cnt = 1
                block_insight_vis={}
                block_insight_vis['Block_index']=block_cnt
                block_insight_vis['son_block']=[]
                block_insight_vis['parent_block']=[]
                block_insight_vis['same-name_block']=[]
                block_insight_vis['siblings_block']=[]                
                # header_info=vis[0].insight_location
                block_insight_vis['Block_size']=cal_block_size(vis[0].insight_location)
                block_insight_vis['insight_location(header)']=vis[0].insight_location
                if(self.data_source.source_name == "Console Sales"):
                    location4=[]
                    location4=header2location(vis[0].insight_location)
                    block_insight_vis['insight_location4']=location4
                    block_insight_vis['insight_list']=[]
                    
                for insight in vis: 
                    insight_in_block={}
                    insight_in_block['Insight_index']=insight_cnt
                    insight_in_block['description']=insight.description
                    # insight_in_block['scope_data']=insight.data
                    insight_in_block['insight_category']=insight.insight_category
                    insight_in_block['insight_score']=insight.insight_score
                    insight_in_block['insight_type']=insight.insight_type
                    insight_in_block['vega_json']=insight.vega_json
                    # print(insight.vega_json)
                    block_insight_vis['insight_list'].append(insight_in_block)
                    insight_cnt += 1
                insight_vis_list.append(block_insight_vis)
        
        # 这里insight_vis_list添加了block之间关系的信息
        insight_vis_list_addLink=generate_block_link_new(insight_vis_list)           
                    
        if (self.data_source.source_name == "Console Sales"):      
            result_file = "insight_result/result_list1.txt"
        if (self.data_source.source_name == "Console Sales(cumulative)"):      
            result_file = "insight_result/result_list2.txt"
        
        with open(result_file, 'a', encoding='utf-8') as file: 
            for i in insight_vis_list_addLink:
                file.write(str(i)+'\n')     
        #                 insight_cnt += 1
        ...
        #         if vis:
        #             block_cnt += 1
        #             file.write('=' * 30 + 'Block' + str(block_cnt) + '=' * 30 + '\n')
        #             insight_cnt = 1
        #             for insight in vis:
        #                 file.write('Insight' + str(insight_cnt) + ':\n')
        #                 file.write(f"description: {insight.description}\n"
        #                            f"insight_category: {insight.insight_category}\n"
        #                            f"insight_score: {insight.insight_score}\n"
        #                            f"insight_type: {insight.insight_type}\n"
        #                            f"insight_location: {insight.insight_location}\n"
        #                            f"vega_json: {insight.vega_json}\n")
    ...
    def process_block(self, args):
        ...
        location = (idx, col)
        if (self.data_source.source_name == "Console Sales"):      
            location_file = "insight_result/all_block_location_1.txt"
        if (self.data_source.source_name == "Console Sales(cumulative)"):      
            location_file = "insight_result/all_block_location_2.txt"
        with open(location_file, 'a', encoding='utf-8') as file:
        ...
        insight_list = get_insight(location, block_data, header_split, transformed_state)
        self.insight_list_.append(insight_list)
    ...
    def get_block_data(self, src_data, idx, col):
        ...
        #     block = block.apply(lambda x: x.astype('object'))
        #     block.index = block.index.astype('object')
