...
class Insight:
    def __init__(self, scope_data, breakdown=None, aggregate=None,location= None):
...
def get_insight(header, block_data, header_split, transformed_state):
    ...
    # print("block_data2")
    # print(block_data)
    get_scope_no_breakdown(block_data,header)
    # index header not none, column header not none(the scope includes more than one row or column )
    if header_split!=0 and header_split!=block_data.shape[1]-1:
        # only consider the first layer of headers as breakdown and aggregate 
        get_scope_with_aggregate(block_data, 0, aggregate, header) # index header
        get_scope_with_aggregate(block_data, header_split, aggregate, header) # column header
...
def get_scope_no_breakdown(block_data,location):
    scope_data = None
    ...
    calc_insight(scope_data, None, None, True,location)

def get_scope_with_aggregate(block_data, breakdown, aggregate ,location):
    ...
    calc_insight(scope_data, breakdown, aggregate,no_aggreate=False, location=location)


''' def get_scope_rearrange_old(location, block_data, header_split):
    ...
    get_scope_rearrange_more(scope_data,location)
...
def get_scope_rearrange(header, block_data, header_split):
    ...
    get_scope_rearrange_advanced(origin_data, header_col_name, header_split, 0, 1, header)
    # group by rows
    header_row_name = origin_data.columns[0]
    get_scope_rearrange_advanced(origin_data, header_row_name, header_split, 1, 0, header)

def get_scope_rearrange_advanced(origin_data, header_name, header_split, idx_num, col_num,header):
    # print(header_name)
            ...
            for j in range(i+1, len(grouped_data_processed)):
                for l in range(len(grouped_data_processed[j].columns)):
                    ...
                    if ins_type=='correlation-temporal':  # only merge when temporal data
                        ...
                        save_insight(scope_data_subset, 'compound', ins_type, ins_score,header=header)
    ...
    if len(tmp_corr_list) > 0:
        ...
        save_insight(scope_data, 'compound', 'correlation-temporal', tmp_score,header=header)
            
def save_insight(scope_data, ins_category, ins_type, ins_score, breakdown=None, aggregate=None ,header=None):
    insight = Insight(scope_data, breakdown, aggregate)
    insight.type = ins_type
    insight.score = ins_score
    insight.category = ins_category
    insight.location = header
...
def get_scope_rearrange_more(scope_data,location):
    for i in range(len(scope_data.columns)):
        for j in range(i+1, len(scope_data.columns)):
            scope_data = pd.concat([scope_data.iloc[:,i], scope_data.iloc[:,j]], axis=1)
            calc_insight(scope_data, None, 'compound',no_aggreate=False,location=location)

def calc_insight(scope_data, breakdown, aggregate, no_aggreate=False, location=None):
    ins_type  = ''
    ins_score = 0
    if check_is_temporal(scope_data):
        ins_type, ins_score = calc_shape_insight(scope_data)
        if ins_score>0:
            save_insight(scope_data, 'shape', ins_type, ins_score, breakdown, aggregate,location)
        ins_type, ins_score = calc_outlier_temporal(scope_data)
        if ins_score>0:
            save_insight(scope_data, 'point', ins_type, ins_score, breakdown, aggregate,location)
    else:
