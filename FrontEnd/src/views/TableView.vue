...
<!-- 左侧的几个操作 -->
    ...
    >
      ...
      >
        ...
        <!-- insight highlighted area insight区域背景色变蓝-->
<!--         <g v-for="(area, index) in highlightedAreaList" :key="index">
          <rect
            v-if="area.top != null"
            class="selected-area"
            style="stroke: steelblue; fill: rgb(186, 219, 228); fill-opacity: 40%; stroke-width: 2.5px;"
            
            :x="markWidth + widthRangeList[area.left]"
            :y="markHeight + heightRangeList[area.top]"
            :width="
              widthRangeList[area.right + 1] -
              widthRangeList[area.left]
            "
            :height="
              heightRangeList[area.bottom + 1] -
              heightRangeList[area.top]
            "
        ...
        <g v-for="(area, index) in highlightedAreaList" :key="index">
          <rect
            ...
            :x="markWidth + widthRangeList[area.left]"
            :y="markHeight + heightRangeList[area.top]"
            :width="widthRangeList[area.right + 1] - widthRangeList[area.left]"
            :height="heightRangeList[area.bottom + 1] - heightRangeList[area.top]"
            @mousedown.stop="selectHighlightArea(index, $event)"
          ></rect>
            <!-- 添加条件渲染的白色覆盖层 -->
            <rect
          ...
          <foreignObject
            ...
            :height="heightRangeList[area.bottom + 1] - heightRangeList[area.top]"
          >
            <div xmlns="http://www.w3.org/1999/xhtml" class="chart-container" :id="'chart-container-' + index">
              <!-- 图表将在这里渲染 -->
            </div>  
          </foreignObject>
        </g>
...
export default {
  ...
  data() {
    return {
      ...
      highlightedArea: { top: null, left: null, bottom: null, right: null },
      highlightedAreaList:[],
      mouseOverCell: {
        row: null,
        column: null,
      ...
      selectedAreaIndex: null,  // 用于追踪当前选中的高亮区域
      selectedInsights: []  // 存储选中区域的所有 Vega-Lite 规范
    };
  },

  methods: {   
    ...mapMutations(["UPDATE_DISPLAY_MODE"]),
/*     selectHighlightArea(index) {
   // 如果当前选中的区域再次被点击，取消选中
   if (this.selectedAreaIndex === index) {
      this.selectedAreaIndex = null;
      this.selectedInsights = [];  // 清空已选的图表数据
      this.$bus.$emit("show_selectedInsights", this.selectedInsights); 
    } else {
      this.selectedAreaIndex = index;
      this.extractInsights(index);  // 提取选中区域的所有 Vega-Lite 规范
      console.log("this.selectedInsights")
      console.log(this.selectedInsights)
    }
  }, */
  ...
  this.extractInsights(index);
  // 设置新的选中区域并渲染图表
  this.selectedAreaIndex = index;
  this.renderChart(index);
},

    extractInsights(index){
    if (index != null && this.blocks_information[index]) {
      const copy_blocks_information_index=this.copyObj(this.blocks_information[index])
      const insights = copy_blocks_information_index.insight_list;
      this.selectedInsights = insights.map(insight => insight.vega_json);
      this.$bus.$emit("show_selectedInsights", this.selectedInsights); 
    } 
    else {
  ...
  this.$nextTick(() => {
    ...
    // 深拷贝 `blocks_information` 对应的 `insight_list`
    const insights = JSON.parse(JSON.stringify(this.blocks_information[index].insight_list));
    const temp_block_Insight_list = insights.map(insight => insight.vega_json);

    const rectWidth = chartContainer.clientWidth;
    const rectHeight = chartContainer.clientHeight;

    const vegaSpec = temp_block_Insight_list[0];
    vegaSpec.width = rectWidth - 20;
    vegaSpec.height = rectHeight - 20;
    vegaSpec.autosize = {type: 'fit', contains: 'padding'};

    // 适当调整编码设置
    if (vegaSpec.encoding.x) {
      vegaSpec.encoding.x.axis = {title: null, labels: false};
    }
    if (vegaSpec.encoding.y) {
      vegaSpec.encoding.y.axis = {title: null, labels: false};
    }
    if (vegaSpec.encoding.size) {
      vegaSpec.encoding.size = null;
    }

    // 使用深拷贝的 vegaSpec 执行渲染
    vegaEmbed(chartContainer, vegaSpec, {actions: false}).then((result) => {
      console.log('图表成功渲染');
    }).catch(console.error);
  });
},
  ...
  for (var key in obj) {
    ...
    getAreaStyle(index) {
      return {
        stroke: 'steelblue',
        fill: this.selectedAreaIndex === index ? 'rgb(255, 255, 0)' : 'rgb(186, 219, 228)', // 黄色填充表示选中
        'fill-opacity': '30%',
        'stroke-width': '1.5px'
      };
    },
  ...
  mounted: function () {
    ...
    //   console.log('')
    // })
    this.$bus.$on("show_block_insight_information", (draw_areas) => {
      this.highlightedAreaList = [];
      // 遍历传入的区域列表并逐个添加到highlightedAreaList
      draw_areas.forEach(area => {
        this.highlightedAreaList.push({
          top: area[0],
          bottom: area[1],
          left: area[2],
          right: area[3]
...
//接受app.vue传递过来的blocks——infomation
    this.$bus.$on("pass_blocks_information", (pass_blocks_information) => {
      this.blocks_information = pass_blocks_information;
      // this.$forceUpdate(); // 强制组件重新渲染
    });
...
// [{"top": 4,"bottom": 5,"left": 8,"right":10},{"top": 7,"bottom": 8,"left": 5,"right":11},]
    this.$bus.$on("change-highlight-area", (draw_areas) => {
    // 清空现有列表
    this.highlightedAreaList = [];
    // 遍历传入的区域列表并逐个添加到highlightedAreaList
    draw_areas.forEach(area => {
